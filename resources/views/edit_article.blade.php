...
<h1 class="m-4">Edit an Article</h1>
<br>
<div class="container">
    ...
    <div class="col-xs-12 card p-4"> 
        <div>  
            <button onclick="javascript:history.go(-1)" class="green_butn d-inline float-end">
                Go Back
            </button>
        </div>
        <div>  
            <small class="lft">TITLE:</small>
            <input class="art_ttl" id="ttl1" type="text" maxlength="100" value="{{$art->title}}"><br>    
            ...
            <br>
            <small class="lft">ARTICLE:</small>
            <div class="art_cnt" contenteditable><?php echo html_entity_decode($art->content); ?></div>
            <br>
            <button onclick="saveToDB('{{$id}}')" class="btn btn-info"> <i class="fa fa-save fa-lg"></i> Save </button>
            <br>
        </div>
    </div>
    @endforeach
...
<script>
    ...
    function saveToDB(id){
        ...
        var aid = id;
        var url1 = '/admin/update_article/' + aid;
            $.ajax({
                url: url1,
                type: "POST",
                data: {
                ...
                success:function (data) {
                    ...
                    window.location.href = "/../admin/myarticles";
                    }, 1000);
                }, error: function (data){
                    console.log(data);
                }
